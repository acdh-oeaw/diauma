...
class MapsTest(TestCase):
    ...
    def test_map(self):
        rv = self.client.post(
            reverse("maps:map-create"), {"name": "Middle Earth"}, follow=True
        )
        ...
        rv = self.client.get(
            reverse("maps:map-detail", kwargs={"pk": map_.id}), follow=True
        )
        ...
        rv = self.client.post(
            reverse("maps:map-delete", kwargs={"pk": map_.id}), follow=True
        )
        self.assertContains(rv, "An entry has been deleted.")

    def test_institute(self):
        rv = self.client.post(
            reverse("maps:institute-create"), {"name": "The Asylum"}, follow=True
        )
        self.assertContains(rv, "The Asylum")
        institute = Institute.objects.create()
        rv = self.client.post(
            reverse("maps:institute-update", kwargs={"pk": institute.id}),
            {"name": "Umbrella Corporation", "info": "Cooperate info"},
            follow=True,
        )
        ...
        rv = self.client.post(
            reverse("maps:institute-delete", kwargs={"pk": institute.id}), follow=True
        )
        self.assertContains(rv, "An entry has been deleted.")

    def test_person(self):
        rv = self.client.post(
            reverse("maps:person-create"), {"name": "Carina"}, follow=True
        )
        ...
        rv = self.client.get(
            reverse("maps:person-update", kwargs={"pk": person.id}), follow=True
        )
        ...
        rv = self.client.get(
            reverse("maps:person-detail", kwargs={"pk": person.id}), follow=True
        )
        self.assertContains(rv, "Laura")
        self.assertContains(rv, "The second Asylum")
        rv = self.client.post(
            reverse("maps:person-delete", kwargs={"pk": person.id}), follow=True
        )
        self.assertContains(rv, "An entry has been deleted.")

    def test_place(self):
        rv = self.client.post(
            reverse("maps:place-create"), {"name": "Atlantis"}, follow=True
        )
        ...
        rv = self.client.get(
            reverse("maps:place-update", kwargs={"pk": place.id}), follow=True
        )
        self.assertContains(rv, "hello info")
        rv = self.client.get(reverse("maps:place"), follow=True)
        self.assertContains(rv, "Atlantis")
        rv = self.client.post(
            reverse("maps:place-delete", kwargs={"pk": place.id}), follow=True
        )
        self.assertContains(rv, "An entry has been deleted.")

    def test_reference(self):
        rv = self.client.post(
            reverse("maps:reference-create"), {"name": "Cryptonomicon"}, follow=True
        )
        ...
        self.assertContains(rv, "Cryptonomicon")
        rv = self.client.post(
            reverse("maps:reference-delete", kwargs={"pk": reference.id}), follow=True
        )
        self.assertContains(rv, "An entry has been deleted.")
    ...
    def test_search(self):
        ...
        rv = self.client.post(
            reverse("maps:search"), {"search-term": "never_find_me"}, follow=True
        )
