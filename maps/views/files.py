...
def index(request):
    ...
    for file in scans:
        if not Scan.objects.filter(file="scan/" + file):
            ...
            link = (
                '<a class="button" download target="_blank" href="/media/scan/'
                + file
                + '">'
            )
            link += "Download</a>"
    ...
    for file in files:
        if not File.objects.filter(file="file/" + file):
            orphaned_files_count += 1
            link = (
                '<a class="button" download target="_blank" href="/media/file/'
                + file
                + '">'
            )
            link += "Download</a>"
    ...
    for file in files:
        ...
        if not Scan.objects.filter(file__contains="scan/" + file_name):
            orphaned_files_count += 1
            link = (
                '<a class="button" download target="_blank" href="/media/IIIF/'
                + file
                + '">'
            )
            link += "Download</a>"
    ...
    )  # available space without reserved blocks
    disk_space_values = {
...
def scan_detail(request, pk):
    ...
    return render(
        request,
        "maps/files/scan/detail.html",
        {
            "scan": scan,
            "iiif": (
                iiif if iiif and os.path.isfile(iiif["file_path"] + ".jp2") else None
            ),
            "tables": tables,
    ...
    def get_form(self):
        ...
        if "origin_id" in self.kwargs and "class_name" in self.kwargs:
            if self.kwargs["class_name"] == "map":
                form.fields["file_map"].initial = Map.objects.get(
                    pk=self.kwargs["origin_id"]
                )
            elif self.kwargs["class_name"] == "reference":
                form.fields["file_reference"].initial = Reference.objects.get(
                    pk=self.kwargs["origin_id"]
                )
        return form
...
class ScanCreate(LoginRequiredMixin, SuccessMessageMixin, CreateView):
    ...
    def get_form(self):
        ...
        if "origin_id" in self.kwargs and "class_name" in self.kwargs:
            if self.kwargs["class_name"] == "map":
                form.fields["scan_map"].initial = Map.objects.get(
                    pk=self.kwargs["origin_id"]
                )
            elif self.kwargs["class_name"] == "reference":
                form.fields["scan_reference"].initial = Reference.objects.get(
                    pk=self.kwargs["origin_id"]
                )
        return form
