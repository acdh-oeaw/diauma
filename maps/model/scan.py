...
def scan_size(value):
    ...
    if value.size > limit:
        message = (
            gettext_lazy("Scan size exceeded. Allowed are:")
            + "  "
...
class Scan(BaseModel):
    ...
    scan_type = ManyToManyField(
        Type, blank=True, related_name="scan_type", verbose_name=gettext_lazy("types")
    )
    info = TextField(blank=True)
    scan_date = DateField(blank=True, null=True)
    scan_person = ManyToManyField(
        Person,
        blank=True,
        related_name="scan_creator",
        verbose_name=gettext_lazy("creator"),
    )
    scan_map = ManyToManyField(
        Map, blank=True, related_name="scan_map", verbose_name=gettext_lazy("maps")
    )
    scan_reference = ManyToManyField(
        Reference,
        blank=True,
        related_name="scan_reference",
        verbose_name=gettext_lazy("references"),
    )
