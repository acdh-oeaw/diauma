...
def file_size(value):
    ...
    if value.size > limit:
        message = (
            gettext_lazy("File size exceeded. Allowed are:")
            + " "
...
class File(BaseModel):
    ...
    file_type = ManyToManyField(
        Type, blank=True, related_name="file_type", verbose_name=gettext_lazy("types")
    )
    file_map = ManyToManyField(
        Map, blank=True, related_name="file_map", verbose_name=gettext_lazy("maps")
    )
    file_reference = ManyToManyField(
        Reference,
        blank=True,
